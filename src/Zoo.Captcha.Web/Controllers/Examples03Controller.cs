...
namespace Zoo.Captcha.Web.Controllers
{
    public class Examples03Controller : Controller
    {
        ...
        {
            using (Bitmap image = new Bitmap(1000, 1000))
            {
                using (Graphics g = Graphics.FromImage(image))
                {
                    ...
                    RectangleF[] regions = g.MeasureString("Text on Path", new Font(FontFamily.GenericSerif, 24), new SolidBrush(Color.Red), TextPathAlign.Center, TextPathPosition.CenterPath, 100, 0, myPath);
                    foreach (var region in regions)
                    {
                        g.FillRectangle(new SolidBrush(Color.GreenYellow), region);
                    }
                    g.DrawString("Text on Path", new Font(FontFamily.GenericSerif, 24), new SolidBrush(Color.Red), TextPathAlign.Center, TextPathPosition.CenterPath, 100, 0, myPath);
